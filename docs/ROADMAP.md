...
| BroadcastChannel | `db:<store>` (ein Kanal pro Store, `db:*` für alle) |
...
| Routing | `react-router-dom` v7 (`createBrowserRouter` in `src/lib/router.tsx`) |
...
type Break = {
  ...
  entryId: string
  startedAt: number
  endedAt?: number           // undefined = laufende Pause
  durationSec: number
  createdAt: number
  updatedAt: number
}
...
type Settings = {
  ...
  roundTo: 0 | 1 | 5 | 15 | 30
  invoiceProfile?: { issuerName?: string; issuerAddress?: string; taxRate?: number; taxId?: string; nextInvoiceNumber?: number; iban?: string; bic?: string; bankName?: string; paymentNote?: string }
  lastBackupAt?: number
  features: FeatureFlags     // Schalter: projects, tags, reports, billing, invoicing, breaks, hoursAccount, weeklyView
  targetHoursPerWeek?: number
}
```

**IndexedDB-Schema** (DB `zeiterfassung`, Version 3, `src/lib/db/db.ts`):
...
- `invoices` – key `id`, indexes `byDate` (seit v2)
- `breaks` – key `id`, indexes `byEntryId`, `byStartedAt` (seit v3)
...
- **Phase 7 — Erweiterungen & web-base** ✅ Feature-Flags (`Settings.features`) mit Presets pro Stufe, Pausen während laufender Einträge (`breaks`-Store, DB v3), Rechnungs-Archiv (`invoices`-Store, DB v2), Wochenübersicht (`pages/Week.tsx`), Stundenkonto (Wochen-Soll), Command-Palette (⌘K), Migration auf die web-base-Foundation (`lib/ui`-Shell, `useLiveQuery`)
