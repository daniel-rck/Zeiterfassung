...
import { type CommandItem, CommandPalette } from "./ui/CommandPalette";
...
export function AppShell() {
  ...
  const visible: NavConfig[] = NAV.filter((item) => !item.feature || features[item.feature]);
  ...
  const primaryMobile: MobileNavItem[] = visible.filter((i) => PRIMARY_MOBILE.includes(i.to));
  const overflow: MobileNavItem[] = visible.filter((i) => !PRIMARY_MOBILE.includes(i.to));
  ...
  return (
    <div className="flex h-screen w-full bg-[color:var(--color-surface-0)]">
      ...
      <MobileBottomNav primary={primaryMobile} overflow={overflow} showFab={showFab} />
