...
import type { InvoiceProfile, Project, TimeEntry } from "../types";
...
export function composeInvoice(entries: TimeEntry[], options: ComposeOptions): ComposedInvoice {
  ...
  if (options.groupBy === "day") {
    ...
    for (const e of billable.slice().sort((a, b) => a.startedAt - b.startedAt)) {
      const rate = e.hourlyRateSnapshot ?? options.project?.hourlyRate ?? options.fallbackRate ?? 0;
