...
const CORE_FEATURES: FeatureMeta[] = [
  { name: 'projects', label: 'Projekte', description: 'Einträge nach Projekten gruppieren', icon: <FolderKanban size={15} /> },
  { name: 'tags', label: 'Tags', description: 'Einträge mit farbigen Tags markieren', icon: <TagsIcon size={15} /> },
  { name: 'reports', label: 'Reports', description: 'Auswertungen nach Woche/Monat, CSV-Export', icon: <BarChart3 size={15} /> },
  { name: 'billing', label: 'Abrechnung', description: 'Stundensätze, abrechenbar-Marker, Rundung', icon: <Banknote size={15} /> },
  { name: 'invoicing', label: 'Rechnungen', description: 'Rechnungs-Vorschau, PDF-Export, Archiv', icon: <FileText size={15} /> },
]

const ADVANCED_FEATURES: FeatureMeta[] = [
  { name: 'breaks', label: 'Pausen', description: 'Pausen während eines laufenden Eintrags erfassen', icon: <Coffee size={15} /> },
  { name: 'pomodoro', label: 'Pomodoro', description: '25/5-Modus für den Timer', icon: <Timer size={15} /> },
  { name: 'notifications', label: 'Erinnerungen', description: 'Web-Benachrichtigungen für tägliche Reminder', icon: <Bell size={15} /> },
  { name: 'hoursAccount', label: 'Stundenkonto', description: 'Wochen-Soll, Ist und Überstunden im Blick', icon: <Gauge size={15} /> },
  { name: 'weeklyView', label: 'Wochenübersicht', description: 'Tagesliste Mo–So mit Summen', icon: <CalendarDays size={15} /> },
]
...
export function SettingsPage() {
  ...
  const applyPreset = async () => {
    const ok = await confirm.confirm({
      ...
      description:
        'Setzt alle Funktions-Schalter auf die Voreinstellung dieser Stufe.',
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-[200px_1fr]">
        ...
        <div className="min-w-0 space-y-5">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateField('pomodoro', {
                            ...
                            longBreakMinutes:
                              settings.pomodoro?.longBreakMinutes ?? 15,
                            setsBeforeLongBreak:
                              settings.pomodoro?.setsBeforeLongBreak ?? 4,
                          })
                        }
                      />
                    </Field>
                      ...
                      <Field label="Rundung">
                        <Select
                          ...
                          onChange={(e) =>
                            updateField(
                              'roundTo',
                              Number(e.target.value) as Settings['roundTo'],
                            )
                          }
                        >
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <Field label="Absender-Name">
                      <Input
                        ...
                        onChange={(e) =>
                          updateInvoiceProfile({ issuerName: e.target.value })
                        }
                      />
                    </Field>
                    ...
                    <Field label="Steuer-ID">
                      <Input
                        ...
                        onChange={(e) =>
                          updateInvoiceProfile({ taxId: e.target.value })
                        }
                      />
                    </Field>
                    ...
                    <Field label="IBAN">
                      <Input
                        ...
                        onChange={(e) =>
                          updateInvoiceProfile({ iban: e.target.value })
                        }
                        placeholder="DE…"
                      />
                    </Field>
                    <Field label="BIC">
                      <Input
                        ...
                        onChange={(e) =>
                          updateInvoiceProfile({ bic: e.target.value })
                        }
                      />
                    </Field>
                    <Field label="Bank">
                      <Input
                        ...
                        onChange={(e) =>
                          updateInvoiceProfile({ bankName: e.target.value })
                        }
                      />
                    </Field>
                    <div className="sm:col-span-2">
                      ...
                      >
                        <Textarea
                          ...
                          onChange={(e) =>
                            updateInvoiceProfile({ paymentNote: e.target.value })
                          }
          ...
          {tab === 'data' && (
            <Card padding="md">
              ...
              <p className="mt-4 text-xs text-[color:var(--color-text-3)]">
                Backups sind reine JSON-Dateien. Sie enthalten alle deine
                Einträge, Projekte, Tags und Einstellungen — und nichts darüber
                hinaus.
              </p>
            </Card>
          )}
          ...
          {tab === 'about' && (
            <>
              <Card padding="md">
                <CardHeader title="Tastatur-Shortcuts" action={<Keyboard size={14} className="text-[color:var(--color-text-3)]" />} />
              ...
              <Card padding="md">
                ...
                <p className="text-sm text-[color:var(--color-text-2)]">
                  Zeiterfassung — Open Source, MIT-Lizenz. Alle Daten bleiben
                  lokal in deinem Browser, kein Account, kein Tracking.
                </p>
              </Card>
            </>
          )}
